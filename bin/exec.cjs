...
const logger = {
  success: function (s, isBold) {
    const handler = isBold ? chalk$1.bold.blue(s) : chalk$1.blue(s);
    return console.log(handler)
  },
  warn: function (s, isBold) {
    const handler = isBold ? chalk$1.bold.yellow(s) : chalk$1.yellow(s);
    return console.log(handler)
  },
  error: function (s, isBold) {
    const handler = isBold ? chalk$1.bold.red(s) : chalk$1.red(s);
    return console.log(handler)
  }
};
...
const pkg = JSON.parse(require$$3__namespace.readFileSync('./package.json','utf-8'));
